# Опис запитів

## Агрегатні функції та Групування

### 1) Загальна фінансова статистика

> ```
> SELECT 
>     SUM(amount) AS total_revenue,
>     AVG(amount) AS average_payment,
>     MIN(amount) AS min_payment,
>     MAX(amount) AS max_payment
> FROM Payment;
> ```

Запит використовує функції SUM, AVG, MIN, MAX для таблиці платежів.

### 2) Пацієнти за діагнозами

> ```
> SELECT 
>     d.title AS diagnosis_title,
>     COUNT(p.patient_id) AS patient_count> 
> FROM Patient p
> JOIN Diagnosis d ON p.diagnosis_id = d.diagnosis_id
> GROUP BY d.title
> ORDER BY patient_count DESC;
> ```

Запит об'єднує пацієнтів з діагнозами та групує їх за назвою хвороби.

### 3) Середня тривалість процедур

> ```
> SELECT 
>     pr.title AS procedure_title,
>     AVG(s.duration) AS avg_duration_minutes,
>     COUNT(s.session_id) AS times_performed
> FROM Session s
> JOIN Procedure pr ON s.procedure_id = pr.procedure_id
> GROUP BY pr.title;
> ```

Запит рахує середню фактичну тривалість сеансів у розрізі типів процедур.

### 4) Активні терапевти

> ```
> SELECT 
>     t.name AS therapist_name,
>     COUNT(s.session_id) AS sessions_count
> FROM Session s
> JOIN Therapist t ON s.therapist_id = t.therapist_id
> GROUP BY t.name
> HAVING COUNT(s.session_id) > 1;
> ```

Запит групує сесії по лікарях і фільтрує результат, залишаючи тільки тих, хто провів більше 1 сеансу.

## Використання JOIN

### 5) Детальний журнал сеансів

> ```
> SELECT 
>     s.date,
>     t.name AS therapist,
>     p.name AS patient,
>     proc.title AS procedure,
>     s.status
> FROM Session s
> INNER JOIN Therapist t ON s.therapist_id = t.therapist_id
> INNER JOIN Patient p ON s.patient_id = p.patient_id
> INNER JOIN Procedure proc ON s.procedure_id = proc.procedure_id
> WHERE s.status = 'Completed';
> ```

Запит використовує INNER JOIN для з'єднання 4 таблиць (Session, Therapist, Patient, Procedure).

### 6) Фінансовий звіт по пацієнтах

> ```
> SELECT 
>     p.name,
>     p.phone,
>     COALESCE(SUM(pay.amount), 0) AS total_paid
> FROM Patient p
> LEFT JOIN Session s ON p.patient_id = s.patient_id
> LEFT JOIN Invoice i ON s.session_id = i.session_id
> LEFT JOIN Payment pay ON i.invoice_id = pay.invoice_id
> GROUP BY p.patient_id, p.name, p.phone
> ORDER BY total_paid DESC;
> ```

Запит використовує LEFT JOIN від таблиці Patient до платежів.

### 7) Матриця послуг

> ```
> SELECT 
>     t.name AS therapist,
>     proc.title AS potential_procedure,
>     proc.cost
> FROM Therapist t
> CROSS JOIN Procedure proc;
> ```

Запит робить повний декартовий добуток таблиць Therapist та Procedure.

## Підзапити

### 8) Дорогі процедури

> ```
> SELECT 
>     s.session_id,
>     s.date,
>     pr.title,
>     pr.cost
> FROM Session s
> JOIN Procedure pr ON s.procedure_id = pr.procedure_id
> WHERE pr.cost > (SELECT AVG(cost) FROM Procedure);
> ```

Запит спочатку обчислює середню вартість процедур (SELECT AVG(cost)...), а потім використовує це число для фільтрації основного списку.

### 9) Порівняння віку

> ```
> SELECT 
>     p.name,
>     p.birth_date,
>    (p.birth_date - (SELECT MIN(birth_date) FROM Patient)) AS days_younger_than_oldest
> FROM Patient p;
> ```

Запит для кожного рядка виконує підзапит, що знаходить найменшу дату народження (найстаршого пацієнта), і обчислює різницю.

### 10) Пацієнти без медкарти

> ```
> SELECT name, phone 
> FROM Patient
> WHERE patient_id NOT IN (SELECT patient_id FROM Medical_Record);
> ```

Запит знаходить пацієнтів, чиї ID відсутні в таблиці Medical_Record.

## Комплексна агрегація

### 11) Ефективність терапевтів

> ```
> SELECT 
>     t.name AS therapist_name,
>     t.specialization,
>     COUNT(s.session_id) AS total_sessions,
>     SUM(i.amount) AS total_invoiced_amount
> FROM Therapist t
> JOIN Session s ON t.therapist_id = s.therapist_id
> JOIN Invoice i ON s.session_id = i.session_id
> GROUP BY t.therapist_id, t.name, t.specialization
> ORDER BY total_invoiced_amount DESC;
> ```

Запит поєднує дані про персонал, сесії та виставлені рахунки.